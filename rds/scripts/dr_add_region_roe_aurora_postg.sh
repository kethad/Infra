...
if [ $APP == ROE -a $Environment == PREPROD ]
then
  dr_db_cluster_param_aurorapostgresql=preprod-roe-cluster-aurora-postgresql13
  echo "$dr_db_cluster_param_aurorapostgresql is dr_db_cluster_param_aurorapostgresql "
elif [ $APP == ROE -a $Environment == PROD ]
then
  dr_db_cluster_param_aurorapostgresql=dr-roe-cluster-param-aurapostgres12
  echo "$dr_db_cluster_param_aurorapostgresql is dr_db_cluster_param_aurorapostgresql "
else
...
if [ $APP == ROE -a $Environment == PREPROD ]
then
  dr_db_paramgroup_aurorapostgresql=dr-roe-param-aurora-postgresql13
  echo "$dr_db_paramgroup_aurorapostgresql is dr_db_paramgroup_aurorapostgresql "
elif [ $APP == ROE -a $Environment == PROD ]
then
  dr_db_paramgroup_aurorapostgresql=dr-roe-param-aurapostgres12
  echo "$dr_db_paramgroup_aurorapostgresql is dr_db_paramgroup_aurorapostgresql "
else
   echo "conditional checks fialed for db cluster parameter groups"
fi
...
then
  db_subnet_group_name=dr-usw2-priv-rds-subnet-group-01
  echo "$db_subnet_group_name is db_subnet_group_name "
else
...
then
  ...
  security_group_id=sg-0371be49396b87216
  echo "$security_group_id is security_group_id "
else
