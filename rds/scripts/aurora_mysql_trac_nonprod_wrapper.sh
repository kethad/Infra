...
sudo chmod +x  $dir/trac_aurora_mysql_nonprod_cf.py
...
then
    ...
    echo "Going to execute aurora_postgresql_prod_cf.py python cf script"
   ./trac_aurora_mysql_nonprod_cf.py  $stackname $region $envapp $drclusteridentifier
fi
...
if [[ -n $envapp && -n $buildid && -n $PrimaryWriterEndpoint && -n $PrimaryReaderEndpoint ]]
then

    echo "updating prod spdji-rds-inventory DynamoDB table for the Aurora PostgreSQL Engine"
    $dir/UpdateBuildState.py  AppName=$envapp  BuilID=$buildid ClusterName=$ClusterName PrimaryWriterEndpoint=$PrimaryWriterEndpoint  PrimaryReaderEndpoint=$PrimaryReaderEndpoint DBEngine=$DBEngine DBEngine_Version=$DBEngine_Version  Build_User_Mail_ID=$buildusermailid  DatenTime="$dt"
else
