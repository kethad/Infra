...
DATE=$(date +%d-%^h-%Y)
mkdir -p -m 0755 $log_path/$DATE-$2-$6-$7
...
#for ec2id in $(/usr/local/bin/aws ec2 describe-instances --filter --region $1 Name=tag:Patching,Values=Yes Name=tag:OS,Values=Linux Name=tag:Environment,Values=[$2,PREPROD] Name=tag:Owner,Values=[$3] Name=tag:AZ,Values=[$4] Name=tag:UsedFor,Values="$5" Name=tag:BuildNumber,Values=[673] | grep -i InstanceId | awk -F\" '{ print $4 }')
for ec2id in $(/usr/local/bin/aws ec2 describe-instances --filter --region $1 Name=tag:Environment,Values=[$2] Name=tag:Owner,Values=[$3] Name=tag:AZ,Values=[$4]  Name=tag:UsedFor,Values="$5,Management Server" Name=tag:Patching,Values=Yes Name=tag:OS,Values=Linux | grep -i InstanceId | awk -F\" '{ print $4 }' )
do
...
sed -e "s/\r//g" ${log_path}/stopped_instances/all-$2-$6-stopped-instances-ids-$(date +%Y-%m-%d).log | EMAIL="IndexInfraMBX@spglobal.com" mutt -s "SPDJI AWS EC2 PATCHING : STOPPED INSTANCES on $2 Environment in $6 Region" "InfraNotificationMBX@spglobal.com,IndexInfraMBX@spglobal.com" "IndexInfraMBX@spglobal.com"
