...
DATE=$(date +%d-%^h-%Y)
for ec2id in $(/usr/local/bin/aws ec2 describe-instances --filter --region $1 Name=tag:Environment,Values=[$2,PREPROD] Name=tag:Owner,Values=[$3] Name=tag:AZ,Values=[$4]  Name=tag:UsedFor,Values="$5" Name=tag:Patching,Values=No Name=tag:OS,Values=Linux | grep -i InstanceId | awk -F\" '{ print $4 }' )
do
