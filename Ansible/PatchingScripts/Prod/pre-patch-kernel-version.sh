...
sed -e "s/\r//g" ${log_path}/$DATE/$2/$3/prepatch-$1-$2-mail-body-host-with-kernel-$(date +%Y-%m-%d).txt | EMAIL="IndexInfraMBX@spglobal.com" mutt -s "SPDJI AWS EC2 PATCHING : PRE-PATCH KERNEL VERSION on $1 Environment in $2 Region" "InfraNotificationMBX@spglobal.com,IndexInfraMBX@spglobal.com,SPDJIAppSupport@spglobal.com" "IndexInfraMBX@spglobal.com"
fi
