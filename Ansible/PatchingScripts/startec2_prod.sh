...
then
#sed -e "s/\r//g" ${log_path}/$DATE/all-$2-$6-stopped-instances-ids-$(date +%Y-%m-%d).log | mutt -s "Stopped Instances: $2 $6" "kirubakaran.kannan@spglobal.com"
sed -e "s/\r//g" $log_path/$DATE/all-$2-$6-stopped-instances-ids-$(date +%Y-%m-%d).log | EMAIL="IndexInfraMBX@spglobal.com" mutt -s "SPDJI AWS EC2 PATCHING : STOPPED INSTANCES on $2 $1" "IndexInfraNotification@spglobal.com,IndexInfraMBX@spglobal.com,SPDJIAppSupport@spglobal.com" "IndexInfraMBX@spglobal.com"
mv $log_path/$DATE/all-$2-$6-stopped-instances-ids-$(date +%Y-%m-%d).log $log_path/$DATE/all-$2-$6-stopped-instances-ids-$(date +%Y-%m-%d:%H:%M:%S).log
fi
fi
